...
`src/App.jsx` is the stateful root: it manages routes (`/` public home displaying only 'Rancho MATHCOUNTS' and the NHD/MATHCOUNTS FAQ item, with Chinese toggle and login; `/hello` check-in, `/hw` homework, `/faq` FAQ, `/practice` sandbox, `/teacher`, `/admin`, `/test`, `/review`), user/auth, student passcode session (`mc_student_id`, `mc_passcode`), ratings, history, and bilingual language state (`en` / `zh` via `src/utils/i18n.js`). Check-in (`/hello`) is isolated from the main homepage and only displayed directly at `/hello` or on Chromebook detection (overridable via `?chromebook=1` or `?chromebook=0`).
